...
`proposal/sections/1-mission.typ` already frames the section as a **funnel → hedge → gap → hero** narrative arc. That scaffold is good and the funnel/hedge/gap are already well-drafted. **Keep them.** The `hero` block is empty — that's the real work of this section, and it's where we drop the concrete instrument and the Boston build (large, densely instrumented capture volume that exists as an independent research instument and a validation tool for th ewearable outdoor rig (imu + eye tracking + drone swarm cameras)) and collaborator network of equivalent animal models. Plan below mirrors that scaffold beat-by-beat.
...
5. **The hardware we build.** Ambitious-but-grounded: use existing tech where it's good enough (FreeMoCap, Neuropixels, miniscopes), and **build what doesn't exist yet** — foremost **next-generation [[next-generation-eye-trackers|eye trackers]]**. The framing is the **smartphone-camera thesis**: commodity cameras are now tiny/fast/cheap enough that mobile trackers can reach the eye DOFs no tracker measures — **torsion** (from iris texture; and it's *not optional* — Listing's law breaks under the always-on VOR of natural locomotion, see [[ocular-torsion-and-listings-law]]) and **lens/accommodation** (Purkinje imaging, already shown on consumer cameras — Yates OpenIrisDPI `@yates2025`). Species-spanning, wireless, miniaturized animal trackers too. Concrete Phase-1 hardware deliverables.
6. **What makes it one instrument (name it briefly, don't lecture):** every sensor's output is a [[sensor-grounded-ontology|sensor-grounded]], [[metrology-and-traceability|calibrated & traceable]] measurement of the same canonical entities, so modalities/species/scales are commensurable. The real engineering challenge is [[complexity-management-first-order|complexity management]] — treated as a first-order problem with the composition/entity-component toolkit that enterprise, defense, and video games converged on; that is *how the instrument scales* across [[dimensional-domains|dimensional domains]], and why it needs a world-class software architect ([[personnel|EI]]). Mentioned in a clause, not defended.
7. **Both X-Labs objectives, explicitly:** the instrument = the **platform technology**; the **[[bdfl-governance|FreeMoCap Foundation X-Lab]]** (independent 501c3) = the **new organizational research structure**.
   ...
   *(The fourth field — agentic AI / mobile robotics — is named in the mission statement and **[[say-vs-allude|alluded]]** to here, then developed in [[03-outcomes|Outcomes]]: because the capture is ontology-clean it comes RL-ready, closing the [[the-robotics-ai-loop|instrument → RL → robots → biology loop]]. Keep it a clause in Mission; don't over-claim.)*
...
[[the-physical-instrument-boston-flagship-and-network]] · [[freemocap]] · [[next-generation-eye-trackers]] · [[ocular-torsion-and-listings-law]] · [[comparative-phylogeny-and-ecs]] · [[the-robotics-ai-loop]] · [[complete-empirical-capture]] · [[the-missing-middle]] · [[sensor-grounded-ontology]] · [[metrology-and-traceability]] · [[why-this-cant-be-academic-science]] · [[matthis-corpus]] · [[the-thesis-instrument-not-software]]
