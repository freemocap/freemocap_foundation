...
// PART 1 ΓÇö Need and framing.  Page budget: ~1.0
//
...
// run FreeMoCap on consumer cameras. State it in one or two sentences ΓÇö do not
...
// ΓöÇΓöÇ A2 CANDIDATE HOOKS (pick one / edit freely; delete rejects + this banner after audit) ΓöÇΓöÇ
...
// ΓöÇΓöÇ A6 SITUATION PARAGRAPH ΓÇö the ┬º1 anchor. If accepted, it REPLACES the final sentence of the paragraph above ("This proposal requests Track 1 support to scope..."), which says the same thing without the evidence. V2 guardrail respected: V2 appears as evidence/motivation only. Citations used: eghbal2020 ΓÇö already in the bib and cited in Part 4, no new entry needed. ΓöÇΓöÇ
#suggestion(note: [A6 ΓÇö the anchor paragraph; standing condition (stadium + self-selected knowledge) then the V2 decision window, then the five scoping areas. Mark up freely.])[
FreeMoCap's community has grown far faster than its maintainer base. Eghbal #c("eghbal2020") describes this structure ΓÇö high user growth, low contributor growth, and maintenance knowledge concentrated in a small core team ΓÇö as the "stadium," and documents it as the common structure of mid-scale open source projects rather than a project-specific failure. The stadium shape carries two costs for FreeMoCap: our picture of the community is incomplete, because we see primarily the users who come to us; and the knowledge required to use, teach, and extend the system remains concentrated in the core team, so potential contributors cannot participate without direct access to us. The software's recent full rebuild (V2, currently in alpha) ΓÇö separating the project into domain-specific repositories with a shared internal structure, moving to a client/server architecture, and shipping as a one-click professional desktop application ΓÇö has lowered the technical cost of contribution and opened a decision point: the organizational structures built around the software now will determine whether user growth converts into a sustainable, community-driven ecosystem. The scoping activities supported by this proposal therefore target five areas: the community landscape and its needs; documentation and upskilling pathways that convert user interest into contributor capacity; governance practices that create transparency and productive communication between core maintainers and the wider community; a validation and benchmarking methodology that other laboratories can replicate; and a sustainability model for the maintenance the software will require. ]
// ΓöÇΓöÇ end A6 ΓöÇΓöÇ
