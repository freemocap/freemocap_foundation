...
// Written Proposal — UNCUT RECONSTRUCTION (2026-07-15)
//
// This is a reconstruction of the proposal BEFORE the deadline-day page cuts.
// Plain text = the SUBMITTED 8-page version. Tinted text was cut to fit the
// 8-page limit and is restored here, colored by how far back it came from:
//   first cut  (violet) — in the 10-page full draft, dropped from submission
//   second cut (teal)   — recovered from the 13-page draft
//   third cut  (coral)  — recovered from the 15-page draft
// See helpers/xref.typ (#cut1/#cut2/#cut3) and the legend below the title.
...
#import "helpers/xref.typ": set-figure-supplements, cut-legend
...
#let wip-banner = align(
  ...
  text(
    ...
    fill: rgb("#00000066"),
    weight: "bold",
    smallcaps[DRAFT - Uncut Reconstruction],
  ),
)
...
#let draft-header = [
  ...
  #grid(
    columns: (1fr, 1fr),
    align(left, text(size: 8pt, fill: rgb("#000000ce"), [Status: Uncut reconstruction])),
...
// Legend for the recovered-cut tints (draft-mode only).
#cut-legend()
