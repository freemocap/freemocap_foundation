...
Because every channel is spatially calibrated, temporally synchronized, and expressed in one sensor-grounded ontology, #Dome records are directly consumable by modern reinforcement-learning and robotics stacks  — a measured bone segment and a simulated robot link are the same `RigidBody`. — giving embodied AI and legged robots the real-world sensorimotor corpora, annotated with the explicit uncertainty bounds safe learning requires.
