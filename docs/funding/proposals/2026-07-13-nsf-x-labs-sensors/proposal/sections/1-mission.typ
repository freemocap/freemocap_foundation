...
Because a measured bone segment and a simulated robot link are the same `RigidBody`, and a measured contact force and a simulated one the same quantity, #Dome records drop into modern reinforcement-learning and robotics stacks with minimal reshaping — giving embodied AI the real-world, uncertainty-bounded sensorimotor corpora that safe learning requires and that no current dataset provides.

Because every channel is spatially calibrated, temporally synchronized, uncertainty-traced within a unified sensor-grounded ontology, #Dome records provide ideal training data for internal and external applications. 
